...
- node-agent 在执行前准备阶段失败且尚未回传 `RUNNING` 时，可调用 task-service 的 `dispatch-failed` 回写派发失败；该调用必须携带 `X-Node-Token`，task-service 应校验 token 与任务绑定节点一致；task-service 完成 `QUEUED/FAILED` 状态回写后必须调用 scheduler-service 释放对应预占；
