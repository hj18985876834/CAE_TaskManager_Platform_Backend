...
职责：统一失败类型。task-service 在 `dispatch-failed` 与 `mark-failed` 写入失败字段前必须校验 `failType` 属于该枚举；运行期超时使用 `mark-finished finalStatus=TIMEOUT`，不得通过 `mark-failed failType=TIMEOUT` 写入。
