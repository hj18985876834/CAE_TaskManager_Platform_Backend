...
```
1. 校验节点本地并发是否允许接受
2. 准备任务目录并优先复用已校验 `workdir`
3. 必要时基于 `unpackDir/workDir` 准备输入文件，而不是重新做平台级解压
4. 回传任务已接收并进入运行前准备阶段
5. 回传 `RUNNING`
...
- 节点代理不应再把“解压 ZIP”当作固定步骤，校验阶段已经准备好的 `workdir` 应作为首选执行目录；
...
- `queueReason` 建议按查询时动态计算，不单独写入任务主表
...
其中 `StatusReportRequest` 第一版只用于 `DISPATCHED -> RUNNING`，终态仍通过 `FinishTaskRequest` 或 `FailTaskRequest` 处理。
