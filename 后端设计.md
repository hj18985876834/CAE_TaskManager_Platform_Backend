...
- 若求解器已成功结束，但结果摘要、结果文件或 `mark-finished` 回调持续失败，node-agent 应使用 `mark-failed(CALLBACK_ERROR)` 收口，而不是继续记为普通 `RUNTIME_ERROR`；task-service 在该终态成功落库后应清理已写入的结果摘要与结果文件元数据，避免 `FAILED` 任务继续展示成功结果；
