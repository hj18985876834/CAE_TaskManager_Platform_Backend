...
- 当前实现支持管理员对 `FAILED`、`TIMEOUT` 任务执行手动重试，使其重新回到 `QUEUED`
...
- 节点预占与任务确认之间允许出现并发竞争，因此若任务确认失败，必须释放刚才的节点预占；
...
- 但接口参数、调度记录表 `strategyName / strategy_name` 以及对外文档统一建议使用稳定标识 `FCFS_MIN_LOAD`
