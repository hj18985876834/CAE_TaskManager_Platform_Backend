...
```
1. 校验节点本地并发是否允许接受
2. 准备任务目录并优先复用已校验 `unpackDir`
3. 必要时基于 `unpackDir` 准备输入文件，而不是重新做平台级解压
4. 回传任务已接收并进入运行前准备阶段
5. 回传 `RUNNING`
...
* `commandTemplate` 应保持轻量，只表达主执行命令和必要占位符；
...
* 注册时 `solvers[].solverId` 必须通过 `solver-service` 校验其全局求解器定义存在，未知求解器应拒绝注册，避免写入脏的 `node_solver_capability`；
