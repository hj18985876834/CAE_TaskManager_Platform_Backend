...
* `NodeOfflineFailRequest`：`nodeId / reason`
* `NodeCapacityRequest`：`taskId`

其中 `StatusReportRequest` 第一版只用于 `DISPATCHED -> RUNNING`，终态仍通过 `FinishTaskRequest` 或 `FailTaskRequest` 处理；`NodeOfflineFailRequest` 仅供 scheduler-service 在节点离线补偿场景调用，返回值应表示本次实际发生状态变更的任务数量。
...
* `failTasksByOffline(NodeOfflineFailRequest request)`
...
### `interfaces/internal/InternalNodeCapacityController.java`

职责：供 task-service / scheduler-service 协作管理节点预占

建议方法：

* `reserve(Long nodeId, NodeCapacityRequest request)`
* `releaseReservation(Long nodeId, NodeCapacityRequest request)`
...
* `dispatchToNode(DispatchTaskDTO task, NodeDTO node)`
...
* `markOfflineNodes()` 在把节点标记为 `OFFLINE` 后，应调用 task-service 的 `failTasksByOffline(...)` 统一补偿该节点上的未完成任务
...
* `markOfflineNodes()` 收到 `failTasksByOffline(...)` 返回值后，应按“实际发生状态变更的任务数量”记录补偿审计，而不是按扫描到的任务总数记账
