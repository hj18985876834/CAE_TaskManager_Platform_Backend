...
RUNNING → FAILED / TIMEOUT / CANCELED
...
- 终态原则上应从 `RUNNING` 进入；不应通过接口内部补跳状态，把 `SCHEDULED`、`DISPATCHED` 任务直接置为终态
...
4. 对目标节点执行容量预占
5. 原子确认任务仍处于 QUEUED
6. 抢占成功后写入 nodeId 并标记为 SCHEDULED
7. 调用 node-agent 下发任务
8. 根据派发结果将任务更新为 DISPATCHED / QUEUED / FAILED
9. 写 schedule_record
```
...
- 调度可用条件建议表达为：`runningCount + reservedCount < maxConcurrency`
...
- 节点进入 `RUNNING` 后，由心跳逐步反映真实运行数，调度侧再释放对应预占。
...
* `discardTask(Long taskId, Long userId)`
...
* `register(...)` 负责同步节点事实上报的基础信息与求解器能力
