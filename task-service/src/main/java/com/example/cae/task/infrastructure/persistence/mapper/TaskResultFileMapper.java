...
public interface TaskResultFileMapper {
	@Insert("INSERT INTO task_result_file(task_id, file_type, file_name, storage_path, file_size) VALUES(#{taskId}, #{fileType}, #{fileName}, #{storagePath}, #{fileSize}) " +
			"ON DUPLICATE KEY UPDATE storage_path = VALUES(storage_path), file_size = VALUES(file_size)")
	int insertOrUpdate(TaskResultFilePO po);
	...
	@Delete("DELETE FROM task_result_file WHERE task_id = #{taskId}")
