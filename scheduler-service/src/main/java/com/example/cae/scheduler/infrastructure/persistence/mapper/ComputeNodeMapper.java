...
public interface ComputeNodeMapper {
	...
	@Insert("INSERT INTO compute_node(node_code, node_name, host, status, max_concurrency, running_count, cpu_usage, memory_usage, last_heartbeat_time) VALUES(#{nodeCode}, #{nodeName}, #{host}, #{status}, #{maxConcurrency}, #{runningCount}, #{cpuUsage}, #{memoryUsage}, #{lastHeartbeatTime})")
	int insert(ComputeNodePO po);

	@Update("UPDATE compute_node SET node_name = #{nodeName}, host = #{host}, status = #{status}, max_concurrency = #{maxConcurrency}, running_count = #{runningCount}, cpu_usage = #{cpuUsage}, memory_usage = #{memoryUsage}, last_heartbeat_time = #{lastHeartbeatTime} WHERE id = #{id}")
	int updateById(ComputeNodePO po);

	@Select({
		"<script>",
		"SELECT id, node_code AS nodeCode, node_name AS nodeName, host, status, max_concurrency AS maxConcurrency, running_count AS runningCount, cpu_usage AS cpuUsage, memory_usage AS memoryUsage, last_heartbeat_time AS lastHeartbeatTime, created_at AS createdAt, updated_at AS updatedAt FROM compute_node",
		"<where>",
