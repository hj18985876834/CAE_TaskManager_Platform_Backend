...
public interface ComputeNodeMapper {
	...
	@Insert("INSERT INTO compute_node(node_code, node_name, host, port, node_token, status, max_concurrency, running_count, cpu_usage, memory_usage, last_heartbeat_time) VALUES(#{nodeCode}, #{nodeName}, #{host}, #{port}, #{nodeToken}, #{status}, #{maxConcurrency}, #{runningCount}, #{cpuUsage}, #{memoryUsage}, #{lastHeartbeatTime})")
	int insert(ComputeNodePO po);

	@Update("UPDATE compute_node SET node_name = #{nodeName}, host = #{host}, port = #{port}, node_token = #{nodeToken}, status = #{status}, max_concurrency = #{maxConcurrency}, running_count = #{runningCount}, cpu_usage = #{cpuUsage}, memory_usage = #{memoryUsage}, last_heartbeat_time = #{lastHeartbeatTime} WHERE id = #{id}")
