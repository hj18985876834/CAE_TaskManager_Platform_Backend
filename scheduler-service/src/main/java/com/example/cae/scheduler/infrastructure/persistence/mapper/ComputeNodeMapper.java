...
public interface ComputeNodeMapper {
	...
	@Update("UPDATE compute_node SET node_name = #{nodeName}, host = #{host}, node_token = #{nodeToken}, status = #{status}, max_concurrency = #{maxConcurrency}, running_count = #{runningCount}, cpu_usage = #{cpuUsage}, memory_usage = #{memoryUsage}, last_heartbeat_time = #{lastHeartbeatTime} WHERE id = #{id}")
