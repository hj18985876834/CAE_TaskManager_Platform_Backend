...
public class OpenFoamExecutor extends AbstractSolverExecutor {
	...
	public OpenFoamExecutor(CommandBuilder commandBuilder,
	...
	public boolean supports(ExecutionContext context) {
		return context != null && "OPENFOAM".equalsIgnoreCase(context.getSolverCode());
	}
	...
	protected ExecutionResult doExecute(ExecutionContext context) {
		...
		if (exitCode != 0) {
			throw new BizException("openfoam execute failed with exit code " + exitCode);
		}
