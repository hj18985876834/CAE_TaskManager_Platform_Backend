...
- 在确有必要时，也允许少量作用域选择器，如 `solverCodes`、`taskTypes`，以及受控的 `deriveParam/deriveParams` 用于参数派生；
- 不建议在首版中把大量复杂正则预处理、命令推导、结果收集流程写入 `rule_json`；
...
同时通过唯一约束 `uk_task_file_type_name(task_id, file_type, file_name)` 保证节点重复上报时按“同一任务 + 同类文件 + 同名文件”幂等覆盖，而不是插入重复记录。
