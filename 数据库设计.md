...
管理员手动重试失败任务时，也不需要新增数据库字段；当前实现直接复用同一条 `sim_task`，将 `status` 回到 `QUEUED`，清空 `node_id / fail_type / fail_message / start_time / end_time` 等运行态字段，同时刷新 `submit_time` 为本次重新入队时间，并通过 `task_status_history` 记录一次新的状态流转。若未来需要“自动重试次数上限”能力，再考虑新增 `retry_count`、`max_retry_count` 一类字段。
...
- 当某任务首次在某节点上成功预占时，写入或更新为 `RESERVED`；
- 当该任务派发失败，或成功进入 `RUNNING` 后释放预占时，更新为 `RELEASED`；
...
- 建议对外统一使用稳定策略标识，如 `FCFS_MIN_LOAD`；
