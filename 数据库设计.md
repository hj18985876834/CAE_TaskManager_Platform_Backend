...
| sort_order        | int          | 排序                             |
| rule_json         | text         | 扩展约束JSON（数量/后缀/大小）   |
| description       | varchar(255) | 说明                             |
...
其中 `DISPATCH_ERROR` 用于 scheduler-service 已将任务置为 `SCHEDULED` 或 `DISPATCHED`，但派发过程失败的场景；是否最终回退到 `QUEUED` 还是进入 `FAILED`，由失败是否可恢复决定；`NODE_OFFLINE` 用于节点心跳超时后，中心服务补偿失败该节点上的未完成任务。
...
| unpack_dir   | varchar(500) | 解压后工作目录                     |
...
这套脚本严格对应文档中的数据库拆分方案、表结构和推荐索引。文档明确建议按微服务拆成 `user_db`、`task_db`、`solver_db`、`scheduler_db` 四个库，并包含 15 张核心表。
...
(1, 1, 'dir_system', 'system/**', '*', 'DIR', 1, 1, '{"allowAnyChildren":true}', 'OpenFOAM必需system目录'),
(2, 1, 'dir_constant', 'constant/**', '*', 'DIR', 1, 2, '{"allowAnyChildren":true}', 'OpenFOAM必需constant目录'),
(3, 1, 'dir_zero', '0/**', '*', 'DIR', 1, 3, '{"allowAnyChildren":true}', 'OpenFOAM必需0目录'),
(4, 2, 'main_inp', 'model.inp', 'model.inp', 'FILE', 1, 1, '{"allowSuffix":["inp"]}', 'CalculiX主输入文件');
