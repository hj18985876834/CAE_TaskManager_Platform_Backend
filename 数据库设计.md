...
其中 `DISPATCH_ERROR` 用于 scheduler-service 已将任务置为 `SCHEDULED` 或 `DISPATCHED`，但派发过程失败的场景；是否最终回退到 `QUEUED` 还是进入 `FAILED`，由失败是否可恢复决定；`NODE_OFFLINE` 仅用于节点心跳超时后最终收口为 `FAILED` 的任务。若 `SCHEDULED`、`DISPATCHED` 任务因节点离线补偿回退到 `QUEUED`，则该原因只写入状态历史或调度记录，不写回 `sim_task.fail_type`。
