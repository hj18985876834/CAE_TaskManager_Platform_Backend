...
其中 `DISPATCH_ERROR` 用于 scheduler-service 已将任务置为 `SCHEDULED`，但调用 node-agent 下发失败的场景；`NODE_OFFLINE` 用于节点心跳超时后，中心服务补偿失败该节点上的未完成任务。
...
(1, 'NODE-001', '计算节点-1', 'node-token-001', '127.0.0.1', 'ONLINE', 1, 4, 1, 23.50, 41.20, NOW(), NOW(), NOW()),
(2, 'NODE-002', '计算节点-2', 'node-token-002', '127.0.0.2', 'OFFLINE', 0, 2, 0, 0.00, 0.00, NOW(), NOW(), NOW());
...
- 重点索引包括 `sim_task` 的 `user_id/status/solver_id/profile_id/submit_time`，以及 `task_log_chunk(task_id, seq_no)`、`compute_node(status/enabled/last_heartbeat_time)`、`schedule_record(task_id/node_id)` 等。
