...
为避免重复校验堆积无效文件记录，设计上应保证同一任务当前有效的输入压缩包只有一份。用户重新上传文件时，应覆盖或替换该任务原有的 `ARCHIVE + input_archive` 记录，而不是不断追加。
...
- 重点索引包括 `sim_task` 的 `user_id/status/solver_id/profile_id/submit_time`，以及 `task_log_chunk(task_id, seq_no)`、`compute_node(status/enabled/running_count/reserved_count/last_heartbeat_time)`、`schedule_record(task_id/node_id)` 等。
