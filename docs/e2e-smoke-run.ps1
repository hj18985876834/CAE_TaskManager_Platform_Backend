...
function Assert-Code200 {
    ...
    if ($null -eq $Resp -or $Resp.code -ne 200) {
        throw "$Step failed: response code is not 200. Raw response: $($Resp | ConvertTo-Json -Depth 10)"
    }
}
...
Assert-Code200 -Resp $loginResp -Step "Login"
...
Assert-Code200 -Resp $meResp -Step "Query current user"
...
Assert-Code200 -Resp $createResp -Step "Create task"
...
if (-not [string]::IsNullOrWhiteSpace($UploadFilePath)) {
    ...
    Assert-Code200 -Resp $uploadResp -Step "Upload task files"
...
Assert-Code200 -Resp $validateResp -Step "Validate task"
...
Assert-Code200 -Resp $submitResp -Step "Submit task"
